...
const metrics = [
  { 
    ...
    sublabel: "simulated across the network",
  },
  { 
    value: 34, 
    suffix: " km/h", 
...
const compactMetrics = [
  ...
  },
  {
    ...
    sublabel: "adaptive control score",
  },
];
...
export function MetricsSection() {
  ...
  return (
    <section id="live-data" ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
      <GridBackground />
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-12 gap-8 mb-20 lg:mb-32">
          <div className="lg:col-span-8 lg:col-start-1">
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[140px] font-display tracking-tight leading-[0.95] transition-all duration-1000 ${
              ...
              Real-time
              <br />
              <span className="text-muted-foreground">traffic intelligence.</span>
            </h2>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6 mt-6">
          {compactMetrics.map((metric, index) => (
            <div
              key={metric.label}
              className={`bg-foreground/[0.02] border border-foreground/10 p-8 flex items-center justify-between gap-6 transition-all duration-700 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
              style={{ transitionDelay: `${(index + 3) * 100}ms` }}
            >
              <div>
                <div className="text-base text-foreground mb-1">{metric.label}</div>
                <div className="text-sm text-muted-foreground font-mono">{metric.sublabel}</div>
              </div>
              <div className="text-3xl md:text-4xl font-display tracking-tight">
                <AnimatedNumber end={metric.value} suffix={metric.suffix} prefix={metric.prefix} />
              </div>
            </div>
          ))}
        ...
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          ...
          <span className="text-foreground">Nagpur Network</span>
        </div>
      </div>
    </section>
  );
